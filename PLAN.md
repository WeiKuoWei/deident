...
| `src/policy/signals.mjs` | ~90 | `proposeTier(group, probe)` | The privacy-tiers §3 proposal, and the entry gate: no proposal is exportable. Deny-list, no remote and a git remote all propose `exclude`; a remote additionally sets `admissible`, which the census counts and the refusal lists but which is never an answer. `redact` and `open` are reached only by a typed decision, because `scan` writes its proposal into review.md and reading it back could not tell the two apart. |
